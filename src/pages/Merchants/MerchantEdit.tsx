...
import { Option, MerchantFormValues, ChangeRequestPayload, BranchInfo } from './types'
...
const EditMerchant = () => {
  ...
  const schema = yup.object().shape({
    ...
    branch_code: yup.object().nullable(),
    hdb_staff_code: yup
      .string()
      .max(50, 'Mã NV HDBank không được vượt quá 50 ký tự'),
  ...
  const { data: branchOptions = [], isLoading: isLoadingBranches } = useQuery<
    Option[]
  >({
    queryKey: ['store-branch-info'],
    queryFn: async () => {
      const response = await axiosInstance.get('/v1/admin/store/branch-info')
      if (response.data.status_code === 'ACCEPT') {
        return (response.data.data || []).map((b: BranchInfo) => ({
  ...
  React.useEffect(() => {
    if (storeData) {
      ...
      const matchedBranch = storeData.branch_code
        ? branchOptions.find((b) => b.value === storeData.branch_code) || {
            label: storeData.branch_name
              ? `${storeData.branch_code} - ${storeData.branch_name}`
              : storeData.branch_code,
      ...
      reset({
        ...
        branch_code: matchedBranch,
        hdb_staff_code: storeData.hdb_staff_code || '',
        // Map location fields.
  ...
  const onSubmit = (data: MerchantFormValues) => {
    ...
    if (dirtyFields.branch_code) {
      payload.branch_code = data.branch_code?.value || ''
    }
                ...
                render={({ field }) => (
                  <>
                    ...
                    {errors.branch_code?.message ? (
                      <p className="text-red-500 text-sm">
                        {errors.branch_code?.message?.toString()}
                      </p>
                    ) : null}
                  </>
                )}
                ...
                render={({ field }) => (
                  <>
                    <Input
                      {...field}
                      label="Mã NV HDBank"
                      placeholder="Nhập mã NV HDBank"
                      className="w-full"
                    />
                    {errors.hdb_staff_code?.message ? (
                      <p className="text-red-500 text-sm">
                        {errors.hdb_staff_code?.message?.toString()}
                      </p>
                    ) : null}
                  </>
                )}
