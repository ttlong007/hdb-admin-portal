...
const EditMerchant = () => {
  ...
  const schema = yup.object().shape({
    ...
    management_unit: yup
      .string()
      .max(100, 'Đơn vị quản lý không được vượt quá 100 ký tự'),
  ...
  React.useEffect(() => {
    if (storeData) {
      ...
      reset({
        ...
        management_unit: storeData.management_unit || '',
        // Map location fields.
                ...
                render={({ field }) => (
                  <>
                    <Input
                      ...
                      className="w-full"
                    />
                    {errors.management_unit?.message ? (
                      <p className="text-red-500 text-sm">
                        {errors.management_unit?.message?.toString()}
                      </p>
                    ) : null}
                  </>
                )}
